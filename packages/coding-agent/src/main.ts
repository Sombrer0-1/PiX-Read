...
async function createSessionManager(
	...
	if (parsed.resume) {
		console.error(chalk.red("Error: --resume requires an interactive session picker, which is not available. Use --session <path|id> or --continue instead."));
		process.exit(1);
	}
...
async function promptForMissingSessionCwd(
	...
	return new Promise((resolve) => {
		...
		console.error(formatMissingSessionCwdPrompt(issue));
		rl.question("Continue with this directory? [y/N] ", (answer) => {
			rl.close();
			const confirmed = answer.toLowerCase() === "y" || answer.toLowerCase() === "yes";
			resolve(confirmed ? issue.fallbackCwd : undefined);
		});
	});
}
...
export async function main(args: string[], options?: MainOptions) {
	...
	if (missingSessionCwdIssue) {
		if (process.stdin.isTTY) {
			const selectedCwd = await promptForMissingSessionCwd(missingSessionCwdIssue);
			if (!selectedCwd) {
				process.exit(0);
			}
