...
import {
	pixAgentDir,
	pixAgentExtensionsDir,
	pixAgentPromptsDir,
	pixAgentSkillsDir,
	pixAppendSystemPromptFile,
	pixAuthJsonFile,
	pixMcpJsonFile,
	pixModelsJsonFile,
	pixProjectSessionsDir,
	pixSystemPromptFile,
} from "./pix-paths.js";

/**
 * pi 的 SettingsManager 只用第一个参数（cwd）定位项目级 <cwd>/.pi/settings.json，
 * 其余设置一律写在 agentDir。PiX 不读工作区配置，因此把项目级作用域也固定到
 * PiX 自己的 agent 目录：工作区里恰好存在 .pi/ 时不会改变 PiX 的行为。
 */
export function createPixSettingsManager(): SettingsManager {
	return SettingsManager.create(pixAgentDir, pixAgentDir);
}

/** 会话目录：PiX 内核设置里的 sessionDir 优先，否则用 PiX 目录内的项目会话目录。 */
export function resolvePixSessionDir(cwd: string): string {
	return createPixSettingsManager().getSessionDir() ?? pixProjectSessionsDir(cwd);
}
...
function resolveReadingSkillsDir(): string {
	const fromThisFile = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "..", "resources", "skills");
	const packaged = join(process.resourcesPath, "skills");
	if (app.isPackaged && existsSync(packaged)) {
		return packaged;
	}
	if (existsSync(fromThisFile)) {
		return fromThisFile;
	}
	try {
		const fromAppPath = join(app.getAppPath(), "resources", "skills");
		if (existsSync(fromAppPath)) return fromAppPath;
	} catch {
		// app.getAppPath() is unavailable before ready; fall back to this file.
	}
	return fromThisFile;
}
...
const TAKE_HER_EYES_SYSTEM_PROMPT = [
	"You are takeHerEyes, a precise vision assistant for PiX-Read.",
...
export class SessionBridge {
	...
	async prompt(
		...
		const prepared = await this._preparePromptInput(text, filePaths, clipboardImages, displayText);
	...
	private _applyPiSetting(key: string, value: unknown): PiSettingApplyResult {
		...
		switch (key) {
			...
			case "promptTemplatePaths": sm.setPromptTemplatePaths(Array.isArray(value) ? value as string[] : []); return { reloadRuntime: true };
				case "packages":
				sm.setPackages(value as Array<{ source: string; extensions?: string[]; skills?: string[]; prompts?: string[] }>);
				return { reloadRuntime: true };
		...
		if (clipboardImages && clipboardImages.length > 0) {
			for (let i = 0; i < clipboardImages.length; i++) {
				...
				allAttachments.push({
					...
					name: `截图-${i + 1}.${img.mimeType.split("/")[1] || "png"}`,
					kind: "image",
				});
			}
		}
	...
	private async _createSession(
		...
		const settingsManager = this._createSettingsManager();
		const authStorage = this._authStorage ?? AuthStorage.create(pixAuthJsonFile);
		this._authStorage = authStorage;
		const mcpAdapter = new McpAdapter({ autoDiscover: false, configPaths: [pixMcpJsonFile] });
		this._mcpAdapter = mcpAdapter;
		const resourceLoader = new DefaultResourceLoader({
			cwd,
			agentDir: pixAgentDir,
			settingsManager,
			// 关闭内核自动发现：工作区的 .pi/ 与 .agents/、用户主目录的 ~/.agents/ 都不得注入
			// 扩展（会在本进程执行代码）、技能、提示模板或 AGENTS.md。PiX 自有目录改为显式传入。
			noExtensions: true,
			noSkills: true,
			noPromptTemplates: true,
			noContextFiles: true,
			// 传空字符串会跳过内核的 <cwd>/.pi/SYSTEM.md 发现，只保留 PiX 目录里的可选覆盖。
			systemPrompt: existsSync(pixSystemPromptFile) ? pixSystemPromptFile : "",
			appendSystemPrompt: existsSync(pixAppendSystemPromptFile) ? [pixAppendSystemPromptFile] : [],
			additionalExtensionPaths: existingPaths([pixAgentExtensionsDir]),
			additionalSkillPaths: existingPaths([resolveReadingSkillsDir(), pixAgentSkillsDir]),
			additionalPromptTemplatePaths: existingPaths([pixAgentPromptsDir]),
			extensionFactories: [
				(pi) => { mcpAdapter.register(pi); },
				createPdfToolsFactory(),
			],
			appendSystemPromptOverride: (base) => [...base, READING_ASSISTANT_SYSTEM_PROMPT],
		});
	...
	private _createSettingsManager(): SettingsManager {
		const settingsManager = createPixSettingsManager();
