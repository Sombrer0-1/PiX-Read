...
const agentStatus = ref<"stopped" | "starting" | "running" | "error">("stopped");
...
function setupEventListeners(): void {
	...
	eventUnsubscribers.push(
		api().onAgentEvent((event: AgentSessionEvent) => {
...
async function startSession(projectDir: string): Promise<boolean> {
	agentStatus.value = "starting";
	lastError.value = null;
	setupEventListeners();

	try {
		const result = await api().startSession(projectDir);
		if (!result.success) {
			cleanupEventListeners();
			agentStatus.value = "error";
			lastError.value = result.error || "无法启动会话";
			return false;
		}
		...
		cleanupEventListeners();
		agentStatus.value = "error";
		lastError.value = err instanceof Error ? err.message : "启动会话时发生未知错误";
		return false;
	}
}
...
async function attachToRunningSession(): Promise<boolean> {
	try {
		...
		lastError.value = err instanceof Error ? err.message : "无法接入正在运行的会话";
		return false;
	}
}
...
async function sendPrompt(
	...
	await sendCommandOrThrow({ type: "prompt", message, filePaths, images, displayText });
}

/**
 * Queue a message while the agent is already running (steer), so Enter during
 * streaming does not attempt a second concurrent prompt.
 */
async function sendSteer(
	...
	await sendCommandOrThrow({ type: "steer", message, filePaths, images, displayText });
}
...
async function setModel(provider: string, modelId: string): Promise<void> {
	await sendCommandOrThrow({ type: "set_model", provider, modelId });
...
async function cycleModel(direction: "forward" | "backward" = "forward"): Promise<void> {
	await sendCommandOrThrow({ type: "cycle_model", direction });
	await refreshState();
...
async function setThinkingLevel(level: ThinkingLevel): Promise<void> {
	await sendCommandOrThrow({ type: "set_thinking_level", level });
...
export function useRpc() {
	return {
		agentStatus: computed(() => agentStatus.value),
		...
		isRunning: computed(() => agentStatus.value === "running"),
		isStreaming: computed(() => sessionState.value?.isStreaming ?? false),
		isConnected: computed(() => agentStatus.value === "running"),
		startSession,
