...
/** "你好，帮我…"这类寒暄开头不承载标题信息，摘掉后第一段才是正文。 */
function stripGreeting(text: string): string {
  return text.replace(
    /^(你好|您好|hi|hello|hey|在吗|在么|哈喽|嗨)[啊呀吧呢嘛！!。.，,~～\s]*[,，:：\s]*/i,
    "",
  );
}

function isGreetingOnly(text: string): boolean {
  return /^(你好|您好|hi|hello|hey|在吗|在么|哈喽|嗨)[啊呀吧呢嘛！!。.，,~～\s]*$/i.test(text.trim());
}

export function summarizeSessionText(text: string): string {
  const compact = stripGreeting(stripRequestPrefix(compactText(text)));
  const parts = compact
    .split(/(?:但是|不过|然后|另外|还有|同时|以及|[。！？!?；;，,\n])/)
    .map((part) => stripRequestPrefix(part))
    .filter((part) => part.length > 1 && !isGreetingOnly(part));
